...
def create_eval_input_pipeline(input_queue, image_size, nrof_preprocess_threads, batch_size_placeholder):
	...
	for _ in range(nrof_preprocess_threads):
		...
		filenames = input_queue.dequeue()
		    ...
		    l_image = read_and_process_image(filename[0],image_size=image_size)
		    r_image = read_and_process_image(filename[1], image_size=image_size)

		    l_images.append(l_image)
		    r_images.append(r_image)
		
		images.append([l_images, r_images])
		
	
	l_batch, r_batch = tf.train.batch_join(
                                images,
                                batch_size=batch_size_placeholder,
