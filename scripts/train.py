...
def main(args):
     ...
     with tf.Graph().as_default():
         ...
         y = tf.placeholder(tf.float32, (None, args.disparity_range), name='y')
         ...
         input_queue = data_flow_ops.FIFOQueue(capacity=100000,
                                               ...
                                               shapes=[l_shape, r_shape, (args.disparity_range)],
                                               shared_name=None, name=None)

         enqueue_op = input_queue.enqueue_many([left_images,right_images, y], name='enqueue_op')

         l_batch, r_batch, labels_batch = input_queue.dequeue_many(args.batch_size)

         #tf.summary.image("right_patches",r_batch[0])
         #tf.summary.image("left_patches",l_batch[0])

         l_branch = model.inference(l_batch,is_training=True,reuse=False)
         r_branch = model.inference(r_batch,is_training=True,reuse=True)
         ...
         staged_lr = [args.learning_rate * x for x in [1, 0.2, 0.04, 0.008]]
	 ...
	 absolute_diff = tf.Print(absolute_diff, [absolute_diff],"absolute_diff")
	 accuracy = tf.reduce_mean(correct_prediction)
         ...
         with sess.as_default():
             ...
             while it < args.max_iter:
                ...
                train_loss,_,acc = sess.run([loss, train_op,accuracy]) #, feed_dict={learning_rate_placeholder:args.learning_rate, batch_size_placeholder: args.batch_size, train_phase:True})
                losses.append(train_loss)

               

                #if it % 100 == 0:
                print("[Iteration {:^5}] Loss : {:^5} Accuracy : {:^5}".format(it,train_loss,acc))
		...
		if it % 100 ==0:
                    ...
                    summary = tf.Summary()
                    summary.value.add(tag='train/loss', simple_value=np.mean(losses))
